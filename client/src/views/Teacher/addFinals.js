...
export default function AddFinal(props) {
...
//   };
  ...
  const submitForm = (e) => {
    ...
    setBtnDisable(true)
    if (file === null) {
        cancelDetele("You must upload Final Exam")
        return null;
    }
    if(sol === null){
        cancelDetele("You must upload Final Solution")
        return null;
    }
    else{
        successAlert()
                    ...
                    <div>
                      {sol === null ? (
                        <Button {...addButtonProps} onClick={() => handleClick("sol")} color="primary">
                          {avatar ? "Upload Result" : "Upload Final Solution"}
                        </Button>
                      ) : (
                        <span>
